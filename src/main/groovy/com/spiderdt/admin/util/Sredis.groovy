...
public class Sredis {
    ...
    Sredis(String host,int port,int timeout){
        Sredis(host,port,timeout,prefix)
	}
	Sredis(String host,int port,int timeout,String prefix){
		this.host = host
		this.port = port
	...
	public String getData(String key, String field) {
		...
		try {
			redisClient = getJedis();
			data = redisClient.hget(prefix+"::"+key, field);
		} catch (Exception e) { // 销毁对象
